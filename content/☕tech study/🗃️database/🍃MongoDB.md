...
# Terminology
- `cluster` - filing cabinet
	- the actual set of servers that hosts your data
	- the physical hardware (or virtual servers in the cloud) that runs MongoDB.
	- **Analogy:** The entire filing cabinet itself.
	- **What it does:** It handles the heavy lifting like performance, security, and backups. When you get a "connection string" from Atlas, you're getting the address of your filing cabinet.
- `database` (db) - drawer in cabinet
	- You can have multiple databases on one cluster to keep different applications or environments separate (e.g., a `blog` database and an `ecommerce` database).
	- An isolated container for your data _inside_ a cluster.
	- **Analogy:** A single drawer in the filing cabinet. You might have one drawer for "HR" and another for "Sales."
	- **How you use it:** The command `use blog` is like choosing to open the "blog" drawer.
- `collection` - A Folder in the Drawer
	- Lives _inside_ a database and holds a group of related documents -> the equivalent of a "table" in a traditional SQL database
	- - **Analogy:** A labeled folder inside a drawer. In your "blog" drawer, you'd have a folder for `posts`, another for `users`, and maybe one for `comments`.
	- **How you use it:** The command `db.posts.find()` tells MongoDB to look inside the currently open drawer (`db`) and find the folder named `posts` to search through.
- `Document` - A Piece of Paper in the Folder
	- A single record inside a collection -> where your actual data lives
	- stored in a JSON-like format (technically BSON) -> structure of field-value pairs
	- analogous to a **row** in a SQL table
	- **Analogy:** A single piece of paper within a folder. Each document is one blog post, one user profile, or one comment.
    
#### full hierarchy
- You connect to your **Cluster** (the filing cabinet).
    - You select a **Database** with `use blog` (you open a drawer).
        - You query a **Collection** with `db.posts.find()` (you look in a folder).
            - This returns a set of **Documents** (the papers inside).

# MongoDB Compass
- the official graphical user interface (GUI) for MongoDB, providing a visual and user-friendly platform to interact with and manage MongoDB databases, collections, and documents without needing to write complex code
# Mongo shell
