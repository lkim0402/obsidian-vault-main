...
>- It's a [[Databases (AWS)#Self-hosted vs Managed|managed]] SQL database service. (related: [[SQL (Relational) vs NoSQL (Non Relational)]]), so it's less complicated than manually configuring an [[EC2]], for example
...
- configure db [[Backend & Main Components Explained#Server|server]]
