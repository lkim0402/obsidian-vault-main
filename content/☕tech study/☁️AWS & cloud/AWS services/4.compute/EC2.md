...
>- Allows you to rent a remote [[Backend & Main Components Explained#Server|server/computer]]
...
- It is a [[Cloud Deployment & Service Models#Infrastructure as service (IAAS)|IAAS]]
	- Your [[Cloud Deployment & Service Models#Infrastructure as service (IAAS)|unit of consumption]] is the instance, which is an [[Operating systems (OS)|operating system]] configured in some way
...
- A private service (runs in [[3 Network Zones (imp)#Public VS Private Services|private AWS zone]]) - uses [[Virtual Private Cloud (VPC)|VPC]] networking by default
	- An EC2 instance is configured to launch in a single VPC subnet, which is in a specific [[⭐ AWS Infrastructure (Regions, AZs, Edge locations)#Availability Zone (AZ)|AZ]]
	- This means that an EC2 instance is AZ-resilient - if an AZ fails, then the EC2 fails 
- On demand billing - per sec/hr depending on the instance
...
- If you click, you can get more info about it
	- public IPv4 address (typically added automatically) which you could use for sending an [[HTTP requests in Express|HTTP request]] to that instance, in case HTTP traffic is allowed
- Connect to instance
	- more on it below
...
- Savings Plan
	...
	- **Pricing depends on the amount of compute usage ($/hour) rather than a specific instance type.**  
		- You commit to spending a fixed amount (e.g., $10/hour) on compute usage for **1 or 3 years**, but you can use **any instance type, region, or even AWS services like [[Fargate]] or [[☕tech study/☁️AWS & cloud/AWS services/4.compute/Lambda]]**.
	- Discounted price
