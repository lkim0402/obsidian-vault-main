...
- We're trying to reach out from our [[Backend & Main Components Explained#Server|server]] to another resource on the internet -> Axios makes responses very simple
- No need to use `Json.parse()`
- makes error handling easier
- Has an alias for the commonly used [[HTTP requests in Express|HTTP request methods]] 
